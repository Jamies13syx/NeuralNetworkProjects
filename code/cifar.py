...
class CIFAR(nn.Module):
    ...
    def __init__(self, specs, dropout=0.0):
        ...
        H, W, C0, C1, C2, C3, kernel_size, F1, F2, F3, F4, nClasses, padding = specs
        pooling = 2
        stride = 1
        dilation = 1
        ...
        self.linear2 = nn.Linear(F2, F3)
        self.linear3 = nn.Linear(F3, F4)
        self.linear4 = nn.Linear(F4, nClasses)
    ...
    def forward(self, prev):
        ...
        prev = self.linear3(prev)
        prev = self.non_linear(prev)
        prev = self.dropout(prev)

        prev = self.linear4(prev)
        return prev
...
def train_CIFAR(model, train_loader, test_loader):
    ...
    for e in range(nEpochs):
        ...
        log_message(log_file, "%3d %8d %8.3f %8.3f %8.3f %8.1f     %6.1f" % (
        e, (e + 1) * total_train_images, total_train_loss / total_train_images, w_loss / w_images,
        test_loss / test_images, 100 * nCorrect / test_images, (time.time() - start)))
...
if __name__ == "__main__":
    ...
    specs = [H, W, C0, C1, C2, C3, kernel_size, F1, F2, F3, F4, nClasses, padding]
